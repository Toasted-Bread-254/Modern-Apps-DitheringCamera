...
// ─────────────────────────────────────────────────────────────────────────────
// New data-driven overloads.
//
// These accept the caller-collected `data: List<T>` directly instead of going
// through the reflective `DatabaseViewModel.data<T>()` machinery. Apps migrate
// to these one at a time; the legacy `viewModel: DatabaseViewModel` overloads
// above stay around until every caller is converted, then they (and
// DatabaseViewModel / TrueDao / etc.) get deleted in a cleanup commit.
//
// Disambiguation: the parameter at position 2 is `data: List<T>` here vs
// `viewModel: DatabaseViewModel` above, so Kotlin overload resolution picks
// the right one based on the actual call-site type.
// ─────────────────────────────────────────────────────────────────────────────
