...
class CompletedLevelsRepository(context: Context) {
    ...
    fun incrementTotalPipesPlaced() {
        val current = prefs.getInt("total_pipes", 0)
        prefs.edit { putInt("total_pipes", current + 1) }
    }

    fun getTotalPipesPlaced(): Int = prefs.getInt("total_pipes", 0)
}
